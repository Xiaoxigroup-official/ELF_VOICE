...
def create_rounded_pixmap(source_pixmap_path, target_size_int, inner_content_ratio=DEFAULT_AVATAR_CONTENT_RATIO):
    ...
    if actual_content_diameter_in_source <= 0:
        ...
        scaled_source_fb = source_pixmap.scaled(target_qsize, Qt.AspectRatioMode.KeepAspectRatioByExpanding, Qt.TransformationMode.SmoothTransformation) # PyQt6 style
    ...
    scaled_source = source_pixmap.scaled(QSize(int(round(scaled_source_width)), int(round(scaled_source_height))),
                                         Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation) # PyQt6 style
...
class VoiceAssistantUI(QWidget):
    ...
    def __init__(self):
        ...
        if os.path.exists(background_image_path_for_paint):
            ...
            if self.main_background_pixmap.isNull(): self.main_background_pixmap = None
        self.init_ui()
    ...
    def paintEvent(self, event):
        ...
        try:
            if self.main_background_pixmap and not self.main_background_pixmap.isNull():
                scaled_pixmap = self.main_background_pixmap.scaled(self.size(), Qt.AspectRatioMode.KeepAspectRatioByExpanding, # PyQt6 style
            ...
            painter.end()
        super().paintEvent(event) # Corrected: Call super().paintEvent for proper event handling
    ...
    def _setup_radio_group(self, parent_layout, group_label_text, options_list, button_group_attr_name,
        ...
        for text in options_list:
            ...
            if text == default_option_text: radio.setChecked(True); target_button_found = True
        if not target_button_found and first_button: first_button.setChecked(True)
    ...
    def init_ui(self):
        ...
        self.subtitle_layout.addWidget(subtitle_label);
        self.subtitle_layout.addSpacing(5)
        checkbox_style = "margin-bottom: 8px; background-color: transparent; color: black;"

        self.cb_show_subtitles = QCheckBox("显示字幕")
        self.cb_show_subtitles.setObjectName("showSubtitlesCheckbox")
        self.cb_show_subtitles.setChecked(True)
        self.cb_show_subtitles.setStyleSheet(checkbox_style)
        self.subtitle_layout.addWidget(self.cb_show_subtitles)

        self.cb_show_english = QCheckBox("显示英文")
        self.cb_show_english.setObjectName("showEnglishCheckbox")
        self.cb_show_english.setChecked(True)
        self.cb_show_english.setStyleSheet(checkbox_style)
        self.subtitle_layout.addWidget(self.cb_show_english)

        left_panel.addLayout(self.subtitle_layout);
        left_panel.addSpacing(25)
        self._setup_radio_group(left_panel, "模型选择", ["本地模型", "在线模型 - Kimi", "在线模型 - DeepSeek"],
        ...
        if self.mic_icon_loaded:
            ...
            self.ready_mic_style = "QPushButton { background-color: transparent; border: none; border-radius: 40px; } QPushButton:hover { background-color: rgba(0,0,0,0.1); }"
        else:
            self.mic_btn.setText("REC");
            self.mic_btn.setFont(QFont("Arial", 10, QFont.Weight.Bold)) # PyQt6 style
            self.ready_mic_style = "QPushButton { background-color: #E0E0E0; color: #333333; border: 1px solid #CCCCCC; border-radius: 40px; } QPushButton:hover { background-color: #D0D0D0; }"
        self.listening_mic_style = "QPushButton { background-color: #79A6FF; border: 3px solid #CDE0FF; border-radius: 40px; color: white; }"
        ...
        self.input_field.returnPressed.connect(self._handle_user_input_submission)
        ...
        self.send_button.clicked.connect(self._handle_user_input_submission)
    ...
    def toggle_listening_state(self):
        ...
        if self.is_listening:
            ...
            if self.mic_icon_loaded:
                self.mic_btn.setIcon(self.mic_q_icon); self.mic_btn.setText("")
            else:
                self.mic_btn.setIcon(QIcon()); self.mic_btn.setText("...")
        else:
            ...
            if self.mic_icon_loaded:
                ...
                self.mic_btn.setIcon(QIcon()); self.mic_btn.setText("REC")

    def _handle_user_input_submission(self):
        user_text = self.input_field.text().strip()
        if not user_text: return
        self.userInputEntered.emit(user_text) # Emitting the signal
